...
async function run() {
  ...
  // Tear down remaining extended planes so monitors cannot pin the event loop on CI.
  for (const id of EXTENDED) {
    await toggleContinuumPlane(config, id, false);
  }

  console.log('plane-toggles: OK');
}

// Force exit: without this, open handles after arm/disarm pin "npm test" for hours on Actions
// (hang observed right after "plane-toggles: OK", 2026-08-01).
run()
  .then(() => process.exit(0))
  .catch((e) => {
    console.error(e);
