...
This deploys Pages then runs `scripts/post-deploy-seo.ps1` (live HEAD checks, meta/AEO spot-check, IndexNow to Bing/IndexNow API). Env URLs are allowlisted https only. Keep `llms.txt`, `sitemap.xml`, `robots.txt`, and `og-card.png` (1200×630 PNG - not SVG) in sync with the release version.
