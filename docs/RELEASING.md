...
- `packages/hub-api/src/server.js` (the `version:` fields returned by `/api/omega/status` and the live map)
...
- `bin/start-stack.js` and `bin/ghost-continuum.js` (banners)
- `landing/index.html`, `landing/llms.txt`, `deploy/jonbailey/hub-preview/index.html`, `deploy/jonbailey/site-seo/llms.txt` (site copy + `?v=`)
- `landing/sitemap.xml` (`lastmod`)
