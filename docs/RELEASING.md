...
**Always, on every public ship.** Do not land code on GitHub or the live site without the rest of this list.

1. Bump `packages/core/src/version.js` and every lockstep file below. `npm test` (includes `test/version-lockstep.js`) must pass.
2. Dated `CHANGELOG.md` section + `release-notes.md` for this version only.
3. One-commit `main` (orphan/amend), author Pitchfork-and-Torch, secret scan + em-dash check.
4. Tag `vX.Y.Z`, GitHub Release with portable zip, **latest release only** (delete older releases/tags).
5. Deploy the public site (`npm run deploy:site` then `npm run deploy:seo`) so ghost.jonbailey.xyz shows the same version.
6. Update `LIVE-SITES.md` + `live-sites-registry.json` `last_known_update` / `version`.

A version bump without a Release, or a Release without a site deploy, is an incomplete ship.

**Always** (operator standing, 2026-08-15): GitHub `/releases/latest`, the live site `softwareVersion`, and `version.js` move together. `scripts/post-deploy-seo.ps1` fails if GitHub latest or the live HTML do not match `VERSION`.
...
real browser. The production alias has a ~5-minute CDN cache. SEO scripts warn when they
fall back to Pages; 403 is expected, but 401 / 429 / 404 / 5xx fail. Optional env
`SEO_BASE` / `SEO_CONTENT_ALIAS` / `SEO_CARD_URL` must be https on those two hosts (no
userinfo).
