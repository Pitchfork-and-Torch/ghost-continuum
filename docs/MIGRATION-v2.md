...
| Area | v1.x | v2.0 |
|------|------|------|
| Command Nexus UI | Multi-panel canvas map | Full **holographic 3D cockpit** (vendored Three.js r160 + canvas fallback) |
...
4. **Three.js** - Command Nexus loads r160 from `/vendor/three.module.js` (no CDN). If WebGL is blocked, the map falls back to **canvas 2D** (focus, shells, pause, resize still work). Do not point the import map at a remote URL.
