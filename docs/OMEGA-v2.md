...
| Asset | Why | Fallback |
|-------|-----|----------|
| **Three.js r160 (vendored)** | True 3D holographic wireframe map from `/vendor/three.module.js` | Canvas 2D projected map in `holo-map.js` if WebGL is blocked |
| **Clash Display / Satoshi (Fontshare, self-hosted)** | Match visual bible typography | System UI fonts |
