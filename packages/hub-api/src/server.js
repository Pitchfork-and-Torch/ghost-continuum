...
export function startHub(config = loadConfig()) {
 ...
 if (isPublicUiAssetPath(url.pathname)) {
 const file = safeUiPath(UI_ROOT, url.pathname.slice(1));
 ...
 if (url.pathname === '/api/ghost-lan' && req.method === 'GET') {
 const lan = await fetchGhostLan(config);
 return json(res, 200, {
 ok: true,
 armed: lan.armed,
 enabled: lan.enabled,
 ...
 dossiers: lan.dossiers || [],
 events: lan.events || [],
 dashboardUrl: lan.dashboardUrl,
 });
 }

 if (url.pathname === '/api/ghost-lan/rotate' && req.method === 'POST') {
