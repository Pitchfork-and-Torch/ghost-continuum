...
const PATTERNS = [
  ...
  { re: /evasion|encoded.?blob|sideload|anti-?emu|packer/i, filter: (e) => scannerSignal(e) || /encoded-blob|sideload|anti-emu|evasion/.test(JSON.stringify(e.detail || {})), label: 'evasion probes' },
...
function scannerSignal(e) {
  ...
  return /scanner|nmap|curl|wget|masscan/.test(ua)
    || ['scanner', 'encoded-blob', 'sideload', 'anti-emu'].includes(e.detail?.probeClass);
}
