...
function classifyTtp(e) {
  ...
  if (/encoded-blob|sideload|anti-emu|uuid-blob|mac-blob|ipv4-blob/.test(blob)) return 'evasion-probe';
...
function buildIncidentBrief(ctx) {
  ...
  lines.push('');
  lines.push('_Defensive only. Ghost Continuum · Luminous Membrane._');
  return lines.join('\n');
}
