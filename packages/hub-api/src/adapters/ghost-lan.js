...
import os from 'os';
import { spawnHidden } from '../../../core/src/silent-spawn.js';
...
function ghostLanRoot(config) {
  return config.paths?.ghostLan;
}

function ghostLanCli(config) {
  return path.join(ghostLanRoot(config), 'bin', 'ghost-lan.js');
}

function ghostLanSentinel(config) {
  return path.join(ghostLanRoot(config), 'src', 'sentinel.js');
}
...
export async function startGhostLanProcess(config) {
  ...
  const root = ghostLanRoot(config);
  spawnHidden(process.execPath, [ghostLanSentinel(config)], { cwd: root });
  for (let i = 0; i < 12; i++) {
...
export async function stopGhostLanProcess(config) {
  ...
  return new Promise((resolve) => {
    const child = spawnHidden(process.execPath, [cli, 'stop'], {
