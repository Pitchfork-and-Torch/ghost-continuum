...
/** Hub-ui files the loopback hub may serve. Vendor Three.js and Fontshare live outside /assets/. */
export const PUBLIC_UI_PREFIXES = ['/assets/', '/vendor/', '/fonts/'];

export function isPublicUiAssetPath(pathname) {
  const p = String(pathname || '');
  if (!p.startsWith('/') || p.includes('..') || p.includes('\\') || p.includes('\0') || p.includes('%')) {
    return false;
  }
  return PUBLIC_UI_PREFIXES.some((pre) => p.startsWith(pre) && p.length > pre.length);
}
