...
export function createOperatorSentinel(opts = {}) {
 ...
 // Compact chip is an X follow link - keep branding while rain/mood chrome still reacts.
 if (tagEl) {
 tagEl.textContent = panel.classList.contains('operator-chip') ? 'FOLLOW ON X' : m.tag;
 }
 if (moodEl) {
 moodEl.textContent = panel.classList.contains('operator-chip') ? '@SUDDENLYJON' : m.label;
 }
 if (img) {
