 ...
 *
 * Dependency: three@0.160 vendored at /vendor/three.module.js only.
 * Canvas 2D if the vendor file is missing or WebGL is blocked. Never a CDN.
 */
...
export async function createHoloMap(container, callbacks = {}) {
 ...
 scene.fog = new THREE.FogExp2(0x0b1018, 0.038);
 state.scene = scene;
 ...
 console.warn('[holo-map] WebGL unavailable, canvas fallback', e);
 return createCanvasFallback(container, callbacks);
 }
 ...
 scene.add(new THREE.AmbientLight(0x5a7a8a, 0.62));
 const key = new THREE.PointLight(0x5ec8c0, 1.05, 42);
 key.position.set(4, 8, 4);
 scene.add(key);
 const rim = new THREE.PointLight(0x8eb8c8, 0.55, 36);
 rim.position.set(-5, 3, -4);
 scene.add(rim);

 // Perspective grid plane
 const grid = new THREE.GridHelper(14, 28, 0x5ec8c0, 0x1a2838);
 grid.position.y = 0;
 ...
 // Soft ambient membrane dust (living fabric feel)
 {
 const count = 180;
 ...
 dust.name = 'membraneDust';
 scene.add(dust);
 state.membraneDust = dust;
 }
 // Axis hint (subtle)
 ...
 color,
 emissive: color,
 emissiveIntensity: 0.42 + (node.glow || 0.5) * 0.28,
 metalness: 0.28,
 roughness: 0.42,
 transparent: true,
 opacity: 0.94,
 ...
 el.style.cssText = `
 position:absolute; pointer-events:none; font-family:Clash Display,sans-serif;
 ...
 const color = parseInt(String(conn.color || '#5ec8c0').replace('#', ''), 16) || 0x00e5ff;
 ...
 color: parseInt(String(s.color || '#5ec8c0').replace('#', ''), 16),
 transparent: true,
 ...
 const fog = data.visual?.fogDensity ?? 0.038;
 scene.fog.density = fog * (state.morph === 'stealth' ? 1.35 : state.morph === 'aggressive' ? 0.85 : 1);
 // morphLight: soft key/rim shift
 const lights = scene.children.filter((c) => c.isPointLight);
 if (lights[0]) {
 lights[0].color.setHex(
 state.morph === 'forensic' ? 0x6bc4a0 : state.morph === 'aggressive' ? 0x9eb8c4 : 0x5ec8c0,
 );
 lights[0].intensity = state.morph === 'stealth' ? 0.7 : state.morph === 'aggressive' ? 1.25 : 1.05;
 }
 if (state.membraneDust?.material) {
 state.membraneDust.material.opacity = state.morph === 'stealth' ? 0.16 : 0.34;
 }
 if (grid?.material) {
 ...
 // Membrane dust drift
 if (state.membraneDust && !reduceMotion) {
 state.membraneDust.rotation.y = t * 0.03;
 const arr = state.membraneDust.geometry.attributes.position.array;
 for (let i = 1; i < arr.length; i += 3) {
 arr[i] += Math.sin(t + i) * 0.0012;
 if (arr[i] > 5) arr[i] = 0.2;
 }
 state.membraneDust.geometry.attributes.position.needsUpdate = true;
 state.membraneDust.material.opacity = state.morph === 'stealth' ? 0.18 : 0.32;
 }
 ...
 e.ring.material.color.set(on ? 0xd4e4e8 : 0x5ec8c0);
 }
 }
 }
...
/** Canvas 2D fallback when vendored Three.js or WebGL is unavailable */
function createCanvasFallback(container, callbacks) {
 ...
 return orbitPaused || reduceMotion;
 }
 ...
 if (prefersStatic()) paint();
 }

 function project(pos, w, h) {
 return projectCanvas(pos, w, h, cam);
 }

 function hitNode(e) {
 const rect = canvas.getBoundingClientRect();
 return hitCanvasNode(
 data?.nodes,
 e.clientX - rect.left,
 e.clientY - rect.top,
 rect.width,
 ...
 drag = beginOrbitDrag(cam, e.clientX, e.clientY);
 canvas.style.cursor = 'grabbing';
 }

 function onPointerMove(e) {
 if (!drag) return;
 cam = moveOrbitDrag(drag, e.clientX, e.clientY).cam;
 paint();
 }

 function onPointerUp(e) {
 if (!drag) return;
 const end = endOrbitDrag(drag, e.clientX, e.clientY);
 drag = null;
 canvas.style.cursor = orbitPaused ? 'default' : 'grab';
 if (!end.click) return;
 onClick(e);
 }
 ...
 function onWheel(e) {
 e.preventDefault();
 cam = zoomOrbit(cam, e.deltaY);
 paint();
 }

 function onDblClick(e) {
 const n = hitNode(e);
 if (n) callbacks.onOpen?.(n);
 }
 ...
 ctx.strokeStyle = 'rgba(94,200,192,0.15)';
 ctx.lineWidth = 1;
 for (let i = -6; i <= 6; i++) {
 ...
 for (const s of visibleShells(data.shells, showShells)) {
 const pts = shellRingPoints(s, w, h, cam);
 if (!pts.length) continue;
 ctx.beginPath();
 ctx.moveTo(pts[0].x, pts[0].y);
 for (let i = 1; i < pts.length; i++) ctx.lineTo(pts[i].x, pts[i].y);
 ctx.closePath();
 ctx.strokeStyle = s.color || '#5ec8c0';
 ctx.globalAlpha = 0.08 + (s.health || 0.5) * 0.1;
 ctx.lineWidth = 2;
 ctx.stroke();
 ctx.globalAlpha = 1;
 }
 ...
 ctx.strokeStyle = c.color || '#5ec8c0';
 ctx.globalAlpha = prefersStatic() ? 0.55 : 0.5 + 0.3 * Math.sin(frame * 0.05);
 ...
 const breath = prefersStatic() ? 0 : Math.sin(frame * 0.04 + (n.position?.x || 0)) * 3;
 const color = n.color || '#5ec8c0';
 ...
 ctx.fill();
 ctx.fillStyle = color;
 ctx.font = '600 9px Clash Display, sans-serif';
 ctx.textAlign = 'center';
 ctx.fillText(n.displayLabel || n.label || n.id, p.x, p.y + breath - r - 4);
 if (n.id === selectedId || n.id === celebrateId) {
 ctx.strokeStyle = '#d4e4e8';
 ctx.globalAlpha = n.id === celebrateId ? 0.95 : 0.85;
 ctx.lineWidth = n.id === celebrateId ? 2.5 : 1.5;
 ctx.beginPath();
 ctx.arc(p.x, p.y + breath, r * 0.95, 0, Math.PI * 2);
 ctx.stroke();
 ctx.globalAlpha = 1;
 }
 }
 ...
 if (!prefersStatic()) {
 frame++;
 if (!drag) cam = idleOrbit(cam, { paused: false, reduceMotion: false, dragging: false });
 }
 paint();
 if (!prefersStatic()) animId = requestAnimationFrame(draw);
 }

 function freezeOrRun() {
 if (prefersStatic()) {
 if (animId) {
 ...
 return {
 setData(d) { data = d; dirty(); },
 setMorph() {},
 setScrub(t) { scrubT = t; dirty(); },
 setOrbitPaused(p) {
 orbitPaused = !!p;
 localStorage.setItem('gc-orbit-paused', orbitPaused ? '1' : '0');
 if (!drag) canvas.style.cursor = orbitPaused ? 'default' : 'grab';
 freezeOrRun();
 return orbitPaused;
 },
 toggleOrbitPaused() {
 ...
 focusThreat() {
 const next = focusThreatCam(cam, data?.nodes || []);
 cam = next.cam;
 if (next.node) selectedId = next.node.id;
 dirty();
 return next.node || null;
 },
 focusNode(id) {
 const next = focusNodeCam(cam, data?.nodes || [], id);
 if (!next.node) return;
 cam = next.cam;
 selectedId = next.node.id;
 dirty();
 },
 resetCamera() {
 cam = resetCam();
 ...
 const ok = applyNodeAppearance(data?.nodes || [], id, patch);
 if (ok) dirty();
 return ok;
 },
 getNode(id) {
 return findNodeById(data?.nodes || [], id);
 },
 clearSelection() {
 selectedId = null;
 dirty();
 ...
 celebrate(nodeId) {
 celebrateId = nodeId || null;
 dirty();
 setTimeout(() => {
 if (celebrateId === nodeId) {
 celebrateId = null;
 dirty();
 }
 }, 2000);
