...
function httpGetJson(url, timeoutMs = 1500, proxyUrl = null) {
  return new Promise((resolve) => {
    ...
    // Hard wall clock: Node req timeout only covers idle-before-headers, not hung bodies.
    ...
    try {
      ...
      req = http.get(url, { timeout: timeoutMs }, (res) => {
        ...
        res.on('end', () => {
          try {
            finish({ ok: res.statusCode === 200, data: JSON.parse(body) });
          } catch {
            finish({ ok: false, data: null });
          }
        });
        res.on('error', () => finish({ ok: false, data: null }));
      });
...
export function stopTrenchCloak(opts = {}) {
  ...
  if (opts.killManaged !== false) {
    // Sync-friendly: do not leave dangling promises that pin the event loop in CI
    try {
      const pid = readManagedPid();
      if (pid) {
        if (process.platform === 'win32') {
        ...
        result.managed = { ok: true, stopped: true, pid };
      } else {
        result.managed = { ok: true, stopped: false };
      }
    } catch (e) {
      result.managed = { ok: false, error: e.message };
    }
  }
  try {
