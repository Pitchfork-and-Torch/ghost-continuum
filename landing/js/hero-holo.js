...
(function () {
  ...
  var DEMO_NODES = [
    { id: 'NEXUS', x: 0.5, y: 0.48, r: 8, c: '#5ec8c0', role: 'hub' },
    { id: 'LAN', x: 0.22, y: 0.35, r: 5, c: '#6bc4a0', role: 'plane' },
    { id: 'EDGE', x: 0.78, y: 0.32, r: 5, c: '#6a9aab', role: 'plane' },
    { id: 'AUDIT', x: 0.28, y: 0.68, r: 4.5, c: '#8eb8c8', role: 'plane' },
    { id: 'GENOME', x: 0.72, y: 0.7, r: 4.5, c: '#9eb8c4', role: 'plane' },
    { id: 'MESH', x: 0.5, y: 0.18, r: 4, c: '#3a9e96', role: 'plane' },
    { id: 'VEIL', x: 0.15, y: 0.52, r: 4, c: '#5ec8c0', role: 'plane' },
    { id: 'MIRAGE', x: 0.85, y: 0.55, r: 4, c: '#9eb8c4', role: 'plane' },
  ];
  ...
  function createCanvasMap(container) {
    ...
    function drawGrid() {
      ...
      g.addColorStop(0, 'rgba(94, 200, 192, 0.07)');
    ...
    function drawLinks(now) {
      ...
      for (var i = 1; i < DEMO_NODES.length; i++) {
        ...
        ctx.strokeStyle = 'rgba(94, 200, 192,' + pulse + ')';
        ctx.lineWidth = 1;
        ctx.beginPath();
    ...
    function drawThreat(now) {
      ...
      if (phase >= 1 && phase <= 3) {
        ctx.strokeStyle = phase >= 2 ? 'rgba(201, 120, 132, 0.55)' : 'rgba(158, 184, 196, 0.5)';
        ctx.lineWidth = 1.5;
        ctx.setLineDash([4, 4]);
      ...
      var col = phase >= 4 ? '#6bc4a0' : phase >= 2 ? '#c97884' : '#9eb8c4';
    ...
    function frame(now) {
      ...
      if (phase === 4) {
        var hub = nodePos(DEMO_NODES[0]);
        ctx.strokeStyle = 'rgba(107, 196, 160, 0.55)';
        ctx.lineWidth = 2;
        ctx.beginPath();
