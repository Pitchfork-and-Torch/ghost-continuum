...
(function () {
  ...
  var MORPHS = {
    stealth: { accent: '#3a9e96', label: 'STEALTH' },
    research: { accent: '#5ec8c0', label: 'RESEARCH' },
    aggressive: { accent: '#9eb8c4', label: 'AGGRESSIVE' },
    forensic: { accent: '#6bc4a0', label: 'FORENSIC' },
  };
  ...
  function ringGauge(svg, pct, color) {
    ...
    var off = c * (1 - pct / 100);
    svg.innerHTML =
      '<circle cx="36" cy="36" r="' + r + '" fill="none" stroke="rgba(94,200,192,0.12)" stroke-width="6"/>' +
  ...
  function init() {
    ...
    function metricsForStep() {
      ...
      if (deceiveSvg) ringGauge(deceiveSvg, deceive, '#6bc4a0');
    }
    ...
    function draw(now) {
      ...
      // grid
      ctx.strokeStyle = 'rgba(94,200,192,0.06)';
      ...
      var tcol = stepIdx >= 3 ? '#6bc4a0' : stepIdx >= 2 ? '#d4a574' : '#c97884';
      ...
      // seal ring
      if (stepIdx >= 3) {
        ctx.strokeStyle = 'rgba(107,196,160,0.7)';
        ctx.lineWidth = 2;
        ctx.beginPath();
      ...
      // soft membrane field particles (calm, not gimmicky)
      if (!reduced) {
        for (var p = 0; p < 14; p++) {
          var ang = (p / 14) * Math.PI * 2 + now * 0.00025;
          var rad = 70 + 18 * Math.sin(now * 0.0012 + p);
          var px = cx + Math.cos(ang) * rad * (w / 320);
          var py = cy + Math.sin(ang) * rad * 0.72 * (h / 220);
          ctx.fillStyle = accent + '33';
          ctx.beginPath();
          ctx.arc(px, py, 1.4 + (p % 3) * 0.35, 0, Math.PI * 2);
          ctx.fill();
        }
      }
    }
