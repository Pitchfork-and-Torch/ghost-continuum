...
$Version = $null
$vf = Get-Content (Join-Path $Root "packages\core\src\version.js") -Raw
if ($vf -match "export const VERSION = '([^']+)'") { $Version = $Matches[1] }
if (-not $Version) {
  Write-Host "FAIL: could not read VERSION from packages/core/src/version.js" -ForegroundColor Red
  exit 1
}
$Base = "https://ghost.jonbailey.xyz"
$PagesDev = "https://ghost-continuum.pages.dev"
...
Write-Host "`n=== pages.dev HTML version (apex can cache ~5 min) ===" -ForegroundColor Cyan
try {
  $pd = (Invoke-WebRequest -Uri "$PagesDev/" -UseBasicParsing -TimeoutSec 30).Content
  if ($pd.Contains($Version)) {
    Write-Host ("  OK  pages.dev contains {0}" -f $Version) -ForegroundColor Green
  } else {
    Write-Host ("  MISS  pages.dev missing {0}" -f $Version) -ForegroundColor Yellow
    $fail++
  }
} catch {
  Write-Host ("  FAIL pages.dev: {0}" -f $_.Exception.Message) -ForegroundColor Red
  $fail++
}
