...
def main() -> None:
    ...
    draw = ImageDraw.Draw(img, "RGBA")
    tx, ty = 390, 145
    ver_src = (ROOT / "packages" / "core" / "src" / "version.js").read_text(encoding="utf-8")
    version = re.search(r"VERSION = '([^']+)'", ver_src).group(1)
    codename = re.search(r"CODENAME = '([^']+)'", ver_src).group(1)
    draw.text((tx, ty), "GHOST CONTINUUM", font=f_kicker, fill=(*TEAL, 255))
    draw.text((tx, ty + 42), f"{codename} - v{version}", font=f_tag, fill=(*AURORA, 255))
